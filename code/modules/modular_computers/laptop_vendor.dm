...
/obj/machinery/lapvend/proc/fabricate_and_recalc_price(var/fabricate = 0)
	...
	if(devtype == 1) 		// Laptop, generally cheaper to make it accessible for most station roles
		...
		switch(dev_cpu)
			if(1)
				if(fabricate)
					fabricated_device.hardware["processor_unit"] = new/obj/item/computer_hardware/processor_unit/small(fabricated_device)
			if(2)
				if(fabricate)
					fabricated_device.hardware["processor_unit"] = new/obj/item/computer_hardware/processor_unit(fabricated_device)
				total_price += 299
		switch(dev_battery)
			if(1) // Basic(750C)
				if(fabricate)
					fabricated_device.hardware["cell"] = new /obj/item/cell/medium(fabricated_device)
			if(2) // Upgraded(1100C)
				if(fabricate)
					fabricated_device.hardware["cell"] = new /obj/item/cell/medium/high(fabricated_device)
				total_price += 199
			if(3) // Advanced(1500C)
				if(fabricate)
					fabricated_device.hardware["cell"] = new /obj/item/cell/medium/super(fabricated_device)
				total_price += 499
		switch(dev_disk)
			if(1) // Basic(128GQ)
				if(fabricate)
					fabricated_device.hardware["hard_drive"] = new/obj/item/computer_hardware/hard_drive(fabricated_device)
			if(2) // Upgraded(256GQ)
				if(fabricate)
					fabricated_device.hardware["hard_drive"] = new/obj/item/computer_hardware/hard_drive/advanced(fabricated_device)
				total_price += 99
			if(3) // Advanced(512GQ)
				if(fabricate)
					fabricated_device.hardware["hard_drive"] = new/obj/item/computer_hardware/hard_drive/super(fabricated_device)
				total_price += 299
		switch(dev_netcard)
			if(1) // Basic(Short-Range)
				if(fabricate)
					fabricated_device.hardware["network_card"] = new/obj/item/computer_hardware/network_card(fabricated_device)
				total_price += 99
			if(2) // Advanced (Long Range)
				if(fabricate)
					fabricated_device.hardware["network_card"] = new/obj/item/computer_hardware/network_card/advanced(fabricated_device)
				total_price += 299
		if(dev_tesla)
			total_price += 399
			if(fabricate)
				fabricated_device.hardware["tesla_link"] = new/obj/item/computer_hardware/tesla_link(fabricated_device)
		if(dev_nanoprint)
			total_price += 99
			if(fabricate)
				fabricated_device.hardware["printer"] = new/obj/item/computer_hardware/printer(fabricated_device)
		if(dev_card)
			total_price += 199
			if(fabricate)
				fabricated_device.hardware["card_slot"] = new/obj/item/computer_hardware/card_slot(fabricated_device)
		if(dev_aislot)
			total_price += 499
			if(fabricate)
				fabricated_device.hardware["ai_slot"] = new/obj/item/computer_hardware/ai_slot(fabricated_device)
		...
		switch(dev_battery)
			if(1) // Basic(300C)
				if(fabricate)
					fabricated_device.hardware["cell"] = new /obj/item/cell/small(fabricated_device)
			if(2) // Upgraded(500C)
				if(fabricate)
					fabricated_device.hardware["cell"] = new /obj/item/cell/small/high(fabricated_device)
				total_price += 199
			if(3) // Advanced(750C)
				if(fabricate)
					fabricated_device.hardware["cell"] = new /obj/item/cell/small/super(fabricated_device)
				total_price += 499
		switch(dev_disk)
			if(1) // Basic(32GQ)
				if(fabricate)
					fabricated_device.hardware["hard_drive"] = new/obj/item/computer_hardware/hard_drive/micro(fabricated_device)
			if(2) // Upgraded(64GQ)
				if(fabricate)
					fabricated_device.hardware["hard_drive"] = new/obj/item/computer_hardware/hard_drive/small(fabricated_device)
				total_price += 99
			if(3) // Advanced(128GQ)
				if(fabricate)
					fabricated_device.hardware["hard_drive"] = new/obj/item/computer_hardware/hard_drive(fabricated_device)
